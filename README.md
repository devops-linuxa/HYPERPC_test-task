...
- Обеспечить кросс‑дистрибутивную поддержку (Debian/Ubuntu, RHEL/CentOS/Rocky).
- Вынести настраиваемые параметры в раздел `defaults/main.yml`.
- Использовать systemd для управления сервисом Nginx.
- Предусмотреть возможность переопределения ключевых параметров через переменные плейбука.
